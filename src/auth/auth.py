...
class InMemoryTokenValidator(TokenValidator):
    ...
    def revoke_token(self, token: str) -> bool:
        """撤销 token"""
        if token in self._token_user_map:
            del self._token_user_map[token]
            return True
        return False


class DatabaseTokenValidator(TokenValidator):
    """数据库 Token 验证器（生产环境使用）"""
    ...
    def validate(self, token: str) -> Optional[UserIdentity]:
        # 实现实际的数据库查询
        # SELECT user_id, user_name, email, role FROM users WHERE token = ?
        raise NotImplementedError("Database validator requires implementation")
...
def get_auth_context(access_token: str, headers: Optional[dict] = None) -> Optional[AuthContext]:
    ...
    device_info = None
    if headers:
        device_info = DeviceIdentity.get_device_info(headers)
